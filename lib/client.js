...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TokenBubbles(props) {
				...
				if (CFG.calibrateWithUsage && outputDelta > 0) {
					...
					if (estTotal > 0) {
						...
						for (const pair of [["text", "output"], ["reasoning", "reasoning"]]) {
							const count = Math.min(CFG.batchCap, Math.floor(sAcc[pair[0]] / CFG.tokensPerSquare));
							sAcc[pair[0]] -= count * CFG.tokensPerSquare;
			...
			// 方块 = 估算 token ÷ tokensPerSquare；余数保留到下次。
			React.useEffect(() => {
				if (sessionId === undefined) return;
				...
				const groups = [];
				for (const kind of ["reasoning", "text"]) {
					const count = Math.min(CFG.batchCap, Math.floor(acc[kind] / CFG.tokensPerSquare));
					acc[kind] -= count * CFG.tokensPerSquare;
