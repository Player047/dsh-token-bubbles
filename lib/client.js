 ...
 *
 * Data sources:
 *  - orange (output) / yellow (reasoning): LIVE — estimated from the
 *    streaming assistant text of the current session snapshot
 *    (chars / charsPerToken), so squares are produced while the reply is
 *    being written.
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CFG = {
			// 每个小方块代表的 token 数：100 或 1000
			tokensPerSquare: 1000,
			// 每行方块数：10 或 20
			columns: 20,
			// 方块边长（px）
			squareSize: 12,
			...
			adaptiveStep: 100,
			adaptiveCap: 50,
			// 流式文本估算：多少字符折合 1 token（与 token-meter 同款启发式）
			charsPerToken: 4,
			// 右下角留白（px）
		...
		/** 流式 partial 的字符数，按块类型分开：{ text, reasoning }。 */
		function streamCountsOf(partial) {
			...
			for (const block of blocks) {
				...
				if (typeof block.text === "string") counts[block.kind] += block.text.length;
			}
			return counts;
		}
		...
		function TokenBubbles(props) {
			...
			// 用量增量 → 蓝色方块入队（输入 → 缓存命中）。输出不走这里，改由流式文本驱动。
			React.useEffect(() => {
				...
				for (const kind of ["input", "cache"]) {
					const count = Math.min(budget, Math.floor(acc[kind] / CFG.tokensPerSquare));
					acc[kind] -= count * CFG.tokensPerSquare;
					budget -= count;
			...
			React.useEffect(() => {
				...
				for (const kind of ["reasoning", "text"]) {
					const delta = counts[kind] - prev[kind];
					if (delta < 0) acc[kind] = 0; // 新一轮/新步骤开始：重置余数
					if (delta <= 0) continue;
					acc[kind] += delta;
					const count = Math.min(CFG.batchCap, Math.floor(acc[kind] / CFG.charsPerToken));
					acc[kind] -= count * CFG.charsPerToken;
